# Ensure that Paperless consume, media and export directories are created with the correct permissions
...
UMBREL_DATA_STORAGE_PAPERLESS_MEDIA_DIR="${UMBREL_DATA_STORAGE_PAPERLESS_DIR}/media"
...
if [[ ! -d "${UMBREL_DATA_STORAGE_PAPERLESS_MEDIA_DIR}" ]]; then
  mkdir -p "${UMBREL_DATA_STORAGE_PAPERLESS_MEDIA_DIR}"
fi
...
set_paperless_correct_permissions "${UMBREL_DATA_STORAGE_PAPERLESS_MEDIA_DIR}"
